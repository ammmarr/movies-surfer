...
function MoviesSearchComponent() {
    const initialMovies = useSelector(state => state.apiData.randomSeries)
    ...
    //       const data = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=801a00d82d2efc5cba24e10087c344d4&language=en-US&query=${query}&page=1&include_adult=false`);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        const data = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=801a00d82d2efc5cba24e10087c344d4&language=en-US&query=${searchRef.current.value}&page=1&include_adult=false`);
        // convert the data to json
        const json = await data.json();
            ...
            <div className='content'>
                ...
                {movies.length > 0 ? <div className='movies-search-ui'>
                    {movies.map((movie: { id: React.Key | null | undefined; backdrop_path: string; title: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; vote_average: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }) =>
                        <Link to={`/movie/${movie.id}`} className="link-container" key={movie.id}>
